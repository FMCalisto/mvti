...
<table width="440" border="0" cellpadding="0" cellspacing="0" height="60" style="border-bottom-style: dotted;border-bottom-width: 1px;border-bottom-color: #CCCCCC;" align="center">
  <!--DWLayoutTable-->
        <tr class="Texto" valign="middle">
          ...
          <td width="57" align="center" valign="middle"><a href="javascript:openpopup('/includes/PedirOrcamento.asp?CLStamp=<%=clstamp%>&ProdutoID=<%=ProdutoID%>')"><img src="/images/mail.gif" alt="Ver mais detalhes" border="0"/><br />Pedir Orçamento</a></td>
        </tr>
</table>
<%End Function%>
...
'Qdo é filtrado por stock tenho de tambem aplicar esta comdição às COMBO
...
if Filtro<>"-1" Then
	REM é porque entrou FABRICANTEID
...
if Filtro="-1" Then
	...
	if Filtro<>"-1"  Then 
		'teste("CAT1")
		REM é porque entrou CATEGORIA1
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" height="60" style="border-bottom-style: dotted;border-bottom-width: 1px;border-bottom-color: #CCCCCC;">
	...
	<tr>
		...
		<td class="Texto" width="50%"  align="right"><div class="Titulo_laranja">Ordenação&nbsp;:&nbsp;
		...
		<%
			REM a PARTE DOS INDICE não pode ser aqui processada. SORTx tem de ser removido
			AInt=instr(MySQL,CampoDeOrdenacao) 'Encontro o campo de ordernacao na string
			If AInt<>0 Then
				Bint=instr(Aint,MySQL," ") 'procuro o proximo espaço em branco
				MySQL=Left(MySQL,AInt-1) & " 2=2 " & Mid(MySQL,Bint+1,len(MYSQL))
			End if

			REM a PARTE DOS INDICE não pode ser aqui processada. Filtro_Stock tem de ser removido
...
Dim Valor : Valor = -1 'Forço a entrada no ciclo
...
Dim oSMTP  
  ...
  Set oSMTP = Server.CreateObject("OSSMTP.SMTPSession")
    ...
    oSMTP.SendTo = MailTo
	If CCto<>"" Then
		oSMTP.CC="WebMaster MINITEL <webmaster@minitel.pt> , "  & CCto
	Else
