<%@LANGUAGE="VBSCRIPT" CODEPAGE="1252"%>
<!--#INCLUDE virtual="/includes/inc_variaveis_globais.asp"-->
<!--#INCLUDE virtual="/includes/inc_JAVASCRIPT.asp"-->
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<title>WebAdmin</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<META NAME="creationdate" CONTENT="Janeiro 2005">
<META NAME="webmaster" CONTENT="Netcetera, it@minitel.pt">
<META NAME="Author" CONTENT="Netcetera (it@minitel.pt)">
<META NAME="Description" CONTENT="Minitel - Sociedade de Fomento de Aplicações Informáticas, L.da.">
<link href="/WebAdmin/Backoffice.css" rel="stylesheet" type="text/css">
</head>
<body topmargin="0" leftmargin="0">
<table border="0" cellpadding="0" cellspacing="0" width="800" height="600">
	<tr>
	  <td width="600" height="100" colspan="2">
	  		<a href="/index.asp"><img border="0" src="/WebAdmin/Images/header.jpg" width="800" height="133"></a>
	  </td>
	</tr>
	<tr valign="top">
	  <td width="150" height="500" valign="top" bgcolor="#E8E8E8" align="center">
			<!--#INCLUDE VIRTUAL="/WebAdmin/includes/inc_gera_menus.asp"-->
	  </td>
	  <td width="450" height="500" align="center" valign="top">
			<!--#INCLUDE virtual="/WebAdmin/includes/inc_Lista_rma.asp"-->
	  </td>
	</tr>
</table>
</body>
</html>
